...
<form  method="post" action="{{route('post.update_reapplication',['id'=>$posts->id])}}">
  ...
  {{-- hiddenでステータスを隠す --}}
   ...
   <div class="form-group col-1 mb-2">
     ...
     <input class="form-control" style="width: 130px" type="text" name="" value="{{$posts->id}}">
 </div>
   ...
   <div class="form-group col-1 mb-2">
       ...
       <input class="form-control" style="width: 130px" type="text"  name="application_day" value="{{$posts->application_day}}" style="width: 100px";>
   </div>
   ...
   <div class="form-group col-4 mb-2">
        ...
        <input class="form-control" style="width: 250px" type="text" name="purchase" value="{{ old('purchase', $posts->purchase) }}">
   </div>
   <div class="form-group col-6 mb-2">
        <label class="label">購入先URL</label>
        <input class="form-control" type="text" name="purchasing_url" value="{{old('purchasing_url',$posts->purchasing_url)}}">
   </div>
   {{-- テキスト入力 --}}
   <div class="form-group col-6 mb-2">
        <label class="label">利用目的</label>
        <input class="form-control" name="purpose_of_use" value="{{old('purpose_of_use',$posts->purpose_of_use)}}">
   </div>

   {{-- 自動表示 --}}
   <div class="form-group col-2 mb-2">
       <label class="label">納品希望日</label>
       <input class="form-control" style="width: 130px" type="date" name="delivery_hope_day" style="width: 125px" value="{{old('delivery_hope_day',$posts->delivery_hope_day)}}">
  </div>
...
<input id="email" class="form-control choice-email" type="email" value="{{ old('purchase', $posts->destination) }}" name="destination" >
</div>
...
<div class="form-group col-6 mb-4">
  ...
  <input class="form-control" name="reason_for_remand" value="{{$posts->reason_for_remand}}" readonly>
</div>

{{-- ボタン配置 --}}
  
<button type="button" class="btn btn-outline-success btn-lg" style="margin: 100px; width: 150px" data-bs-toggle="modal" data-bs-target="#exampleModal" >再申請</button>
...
<button type="button" class="btn btn-outline-danger btn-lg" style="margin: 100px; width: 150px" data-bs-toggle="modal" data-bs-target="#example2Modal">取下</button>
...
<div class="modal fade" id="example2Modal" tabindex="-1" aria-labelledby="example2ModalLabel" aria-hidden="true">
  ...
  <div class="modal-content">
   ...
   <div class="modal-footer">
    {{-- 取下のメールを作成しないと --}}
    <form  method="post" action="{{route('post.destroy_reapplication',['post_id'=>$posts->id])}}">
      @csrf
